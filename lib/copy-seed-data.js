...
function insertRows(tableName, rows) {
    console.log('Adding ' + rows.length + ' rows of seed data to ', tableName);
    return destKnex(tableName).insert(rows)
        .then(function() {
...
function executeQuery(knexQuery) {
    ...
    return knexQuery.then(_.partial(insertRows, tableName));
}
...
function initRawQueries() {
    return config.get('seedDataConfig.rawRows').map(function(rawConfig) {
        var tableName = rawConfig.tableName;
        var rows = rawConfig.rows;
        var fakeKnexQuery = Promise.resolve(rows);
        fakeKnexQuery._single = { table: tableName };
        return fakeKnexQuery;
    });
}
...
function buildMultiTableSeedQuery(knex, queryConfig) {
    ...
    if (queryConfig.where === 'study') {
        ...
        query.where.apply(query, queryConfig.where);
    } else {
...
function init(credentials) {
    ...
    console.log(destConnectionConfig);
    ...
    var knexQueries = initAllRowsSeedQueries(srcKnex)
        ...
        .concat(initRawQueries());
    var affectedTables = _.uniq(knexQueries.map(getTableName));
    ...
    var disableTriggers = function() {
        return setTriggers(destKnex, affectedTables, false);
    };

    var cleanAllTables = function() {
        return Promise.all(affectedTables.map(cleanTable));
    };
